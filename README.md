...
- 새로운 공격 패턴에 적응하면서도 기존 정상/공격 패턴을 유지할 수 있는지

를 검증하는 것이 이번 실험의 목적이다.

토이 단계에서는 **CIC-IDS2017** 데이터셋을 사용해,

1. 클라이언트 측 **비지도 대조 표현 학습 + 적응형 EWC 기반 지속 학습** 모듈,
2. 서버 측 **멀티 에이전트 기반 연합 학습(Federated Learning)** 구조

가 실제로 분포 드리프트 상황에서 얼마나 안정적인 성능을 내는지 테스트했다.
